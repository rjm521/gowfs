...
func (fs *FileSystem) Create(
	...
	header, err := fs.Config.GetBasicAuthInfoHeader()
	if err != nil {
		return false, err
	}
...
// DownLoad 从HDFS中下载文件到本地
func (fs *FileSystem) DownLoad(hdfsPath, localFile string) (bool, error) {
	...
	reader, err := fs.Open(Path{Name: hdfsPath}, 0, 0, 0)
	if err != nil {
		return false, err
	}
...
// Upload 上传本地文件到HDFS
func (fs *FileSystem) Upload(localFile string, hdfsPath string, overwrite bool) (bool, error) {
	if _, err := os.Stat(localFile); os.IsNotExist(err) {
		return false, fmt.Errorf("File %v not found in local", localFile)
	}
